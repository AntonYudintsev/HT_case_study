...
void shuffle (vector<string> &a, int s)
{
  auto rng = default_random_engine(s);
  for (auto i=a.size()-1; i>0; --i)
    swap(a[i], a[std::uniform_int_distribution<>(0,i)(rng)]);
}
...
void RunTest(int count, int capacity)
{
  ...
  printf("\n%s capacity=%d (kinda)memory=%d loadFactor = %f\n", HT::Name(), capacity, capacity * ht.EntrySize, float(count)/capacity);
    ...
    shuffle(keysInclude, 11);
    shuffle(keysExclude, 12);
...
int main()
{
  ...
  RunTest<OA_Simple>(cnt, sz*1.11762);
  RunTest<SC_Struct>(cnt, sz);
  return 0;
}